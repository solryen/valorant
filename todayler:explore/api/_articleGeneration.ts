...
type ArticleGenerationEnv = NodeJS.ProcessEnv;
...
function readEnv(env: ArticleGenerationEnv, ...names: string[]) {
...
function convertTimeZoneWallTimeToUtc(parts: ZonedDateParts, timeZone: string) {
...
function getNextAthensPublishAt(index: number, generationDate: Date) {
...
async function fetchPexelsImage(query: string, env: ArticleGenerationEnv) {
...
export async function generateWeeklyArticles(env: ArticleGenerationEnv = process.env, generationDate = new Date()) {
...
`;
  ...
  const generated = JSON.parse(response.text || '[]') as GeneratedArticleInput[];
...
export function mapGeneratedArticleToRow(article: Article) {
  return {
    id: article.id,
    slug: article.slug,
...
export async function persistGeneratedArticles(env: ArticleGenerationEnv, articles: Article[]) {
...
export async function publishNextQueuedArticle(env: ArticleGenerationEnv = process.env) {
  const supabaseUrl = readEnv(env, 'SUPABASE_URL', 'EXPO_PUBLIC_SUPABASE_URL', 'VITE_SUPABASE_URL').replace(/\/$/, '');
  const serviceRoleKey = readEnv(env, 'SUPABASE_SERVICE_ROLE_KEY', 'VITE_SUPABASE_SERVICE_ROLE_KEY');
  ...
  const queuedArticles = (await listResponse.json()) as Array<{
    id: string;
    slug: string;
    published_at: string;
  }>;
  ...
  const updatedArticles = (await updateResponse.json()) as unknown;
  return {
    published: 1,
...
export async function verifyAdminSession(env: ArticleGenerationEnv, authorizationHeader: string | null) {
  const supabaseUrl = readEnv(env, 'SUPABASE_URL', 'EXPO_PUBLIC_SUPABASE_URL', 'VITE_SUPABASE_URL').replace(/\/$/, '');
  ...
  if (!supabaseUrl || !supabaseAnonKey || !adminEmail) {
    return { ok: false as const, status: 500, message: 'Generation proxy is not configured.' };
  }
  ...
  if (!response.ok) {
    return { ok: false as const, status: 401, message: 'Invalid or expired admin session.' };
  }

  const user = (await response.json()) as { email?: string | null };
  if ((user.email ?? '').trim().toLowerCase() !== adminEmail) {
    return { ok: false as const, status: 403, message: 'This account cannot generate articles.' };
  }
